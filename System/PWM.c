...
void PWM_Init()
{
	...
	//配置自动重装载寄存器的参数，该参数配为：当计数器计数100次时进行重装
	//该操作使得占空比计算的分母为100，分辨率为1%
	TIM_TimeBaseInitStruct.TIM_Period = (100 - 1);//ARR自动重装寄存器
//	//配置预分频器的参数，该参数配为分频7200倍，此时接入计数器的频率变为72MHz/7200=10000Hz
//	//此时配合自动重装载寄存器的参数配置，可以使得计数器1s计数10000次而满足重装载条件，并申请中断
//	//即此配置下计数器1s申请一次中断
	TIM_TimeBaseInitStruct.TIM_Prescaler = (720 - 1);//PSC分频器
	...
	//参考引脚定义图，OC1连接的CH1通道，而CH通道连接的是A0口，故配置A0口进行输出
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0; //GPIO_Pin_15：重映射功能选择口
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOA, &GPIO_InitStructure);

//	/************引脚重映射***************/
//	//此处参考引脚定义图，发现可将上述PWM波形输出的CH1从A0重映射到A15上
//	//重映射功能用到的是AFIO，我们需要开启AFIO的时钟
//	RCC_APB2PeriphClockCmd(RCC_APB2Periph_AFIO, ENABLE);
//	//选择部分重映射模式1：GPIO_PartialRemap1_TIM2，是查阅参考手册8.3.7 定时器复用功能重映射的表43选择出来的
//	GPIO_PinRemapConfig(GPIO_PartialRemap1_TIM2, ENABLE);
//	//现在重映射已经完成，但由于stm32默认使用时候已经对A15复用为JTDI数据传输口（参考引脚定义图知）
//	//我们为了后面A15能是个普通IO口来输出PWM，我们需要解除JTDI对A15的复用
//	//这里注意不要误使用其他参数，把SW口（用来下载程序的）的复用解除了
//	//如：参数GPIO_Remap_SWJ_Disable就会接触SW口的复用功能，导致stm32无法下载程序
//	GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable, ENABLE);
//	//现在就把A0重映射到A15了，这时我们需要将原先对GPIO_Pin_0的初始化转移到15那里
...
{
	//这个函数是在程序运行过程中更改CCR比较寄存器的值用的，在这里体现为更改占空比
